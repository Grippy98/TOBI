...
TOBI-lite keeps HDMI output enabled and uses `tobi.lite=1`, `tobi.ttys=/dev/tty0,/dev/ttyS2`, and `cma=32M` in `uEnv.txt`. HDMI runs the only TOBI app by default; the serial console prints a lightweight prompt explaining that typing `SERIAL` stops the HDMI instance and starts TOBI on serial instead. HDMI mode is not restarted after that handoff. It keeps the AM62-SIP DTB (`k3-am6254atl-sk.dtb`) and prunes the initramfs module tree to the eMMC/SD, USB storage/HID, FAT/ext4, CPSW Ethernet, and DRM/TIDSS HDMI modules needed for installer use.
